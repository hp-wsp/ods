 ...
 */
@Service
public class ImportEvaluationService {
    private static final Logger LOGGER = LoggerFactory.getLogger(ImportEvaluationService.class);
    ...
    @Autowired
    public ImportEvaluationService(EvaItemDao itemDao, TaskCardDao taskCardDao,
                                   TaskCardItemDao taskItemDao, CompanyDao companyDao,
    ...
    public void importEvaluation(Evaluation evaluation, String importId){
    ...
    private void importTask(Evaluation evaluation, String importId, Map<String, EvaItem> evaItemMap,
        ...
        for(TaskCard card: cards){
            ...
            List<TaskCardItem> taskItems = taskItemDao.findByCardId(cardId);
            for(TaskCardItem t: taskItems){
    ...
    private void insertTaskItem(TaskCard card, TaskCardItem taskItem,
        ...
        List<TaskCardItem.TaskItemResult> results = taskItem.getResults().stream()
    ...
    private TaskCardItem.TaskItemResult newTaskItemResult(int totalScore, TaskCardItem.TaskItemResult result, Map<String, Integer> gradeRates){
        ...
        return new TaskCardItem.TaskItemResult(result.getLevel(), score);
    }
}
