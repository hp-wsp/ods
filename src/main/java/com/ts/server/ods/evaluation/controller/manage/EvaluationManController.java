package com.ts.server.ods.evaluation.controller.manage;

import com.ts.server.ods.controller.vo.OkVo;
import com.ts.server.ods.controller.vo.ResultPageVo;
import com.ts.server.ods.controller.vo.ResultVo;
import com.ts.server.ods.evaluation.controller.manage.form.EvaluationSaveForm;
import com.ts.server.ods.evaluation.controller.manage.form.EvaluationUpdateForm;
import com.ts.server.ods.evaluation.controller.manage.logger.EvaluationLogDetailBuilder;
import com.ts.server.ods.evaluation.domain.Evaluation;
import com.ts.server.ods.evaluation.service.EvaluationService;
import com.ts.server.ods.logger.aop.annotation.EnableApiLogger;
import com.ts.server.ods.security.annotation.ApiACL;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import io.swagger.annotations.ApiParam;
import org.apache.commons.lang3.StringUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import javax.validation.Valid;

import java.util.List;

import static org.springframework.http.MediaType.APPLICATION_JSON_UTF8_VALUE;

/**
 * 评测API接口
 *
 * @author <a href="mailto:hhywangwei@gmail.com">WangWei</a>
 */
@RestController
@RequestMapping("/manage/evaluation")
@ApiACL({"ROLE_SYS"})
@Api(value = "/manage/evaluation", tags = "测评API接口")
public class EvaluationManController {
    private final EvaluationService service;

    @Autowired
    public EvaluationManController(EvaluationService service) {
        this.service = service;
    }

    @PostMapping(consumes = APPLICATION_JSON_UTF8_VALUE, produces = APPLICATION_JSON_UTF8_VALUE)
    @EnableApiLogger(name = "新增评测", type = "evaluation", buildDetail = EvaluationLogDetailBuilder.SaveBuilder.class)
    @ApiOperation("新增评测")
    public ResultVo<Evaluation> save(@Valid @RequestBody EvaluationSaveForm form){
        Evaluation evaluation = service.save(form.toDomain(), form.getImportId());
        return ResultVo.success(evaluation);
    }

    @PutMapping(consumes = APPLICATION_JSON_UTF8_VALUE, produces = APPLICATION_JSON_UTF8_VALUE)
    @EnableApiLogger(name = "修改评测",  type = "evaluation", buildDetail = EvaluationLogDetailBuilder.UpdateBuilder.class)
    @ApiOperation("修改评测")
    public ResultVo<Evaluation> update(@Valid @RequestBody EvaluationUpdateForm form){
        Evaluation evaluation = service.update(form.toDomain());
        return ResultVo.success(evaluation);
    }

    @GetMapping(value = "{id}", produces = APPLICATION_JSON_UTF8_VALUE)
    @ApiOperation("得到评测")
    public ResultVo<Evaluation> get(@PathVariable("id")String id){
        return ResultVo.success(service.get(id));
    }

    @DeleteMapping(value = "{id}", produces = APPLICATION_JSON_UTF8_VALUE)
    @EnableApiLogger(name = "删除评测",  type = "evaluation", buildDetail = EvaluationLogDetailBuilder.DeleteBuilder.class)
    @ApiOperation("删除评测")
    public ResultVo<OkVo> delete(@PathVariable("id")String id){
        return ResultVo.success(new OkVo(service.delete(id)));
    }

    @PutMapping(value = "open/{id}", produces = APPLICATION_JSON_UTF8_VALUE)
    @EnableApiLogger(name = "开启评测",  type = "evaluation", buildDetail = EvaluationLogDetailBuilder.OpenBuilder.class)
    @ApiOperation("开启评测")
    public ResultVo<Evaluation> open(@PathVariable("id")String id){
        return ResultVo.success( service.updateStatus(id, Evaluation.Status.OPEN));
    }

    @PutMapping(value = "close/{id}", produces = APPLICATION_JSON_UTF8_VALUE)
    @EnableApiLogger(name = "关闭评测",  type = "evaluation", buildDetail = EvaluationLogDetailBuilder.CloseBuilder.class)
    @ApiOperation("关闭评测")
    public ResultVo<Evaluation> close(@PathVariable("id")String id){
        return ResultVo.success(service.updateStatus(id, Evaluation.Status.CLOSE));
    }

    @PutMapping(value = "openDec/{id}", produces = APPLICATION_JSON_UTF8_VALUE)
    @EnableApiLogger(name = "开启申报",  type = "evaluation", buildDetail = EvaluationLogDetailBuilder.OpenBuilder.class)
    @ApiOperation("开启申报")
    public ResultVo<Evaluation> openDec(@PathVariable("id")String id){
        return ResultVo.success(service.openDec(id));
    }

    @PutMapping(value = "closeDec/{id}", produces = APPLICATION_JSON_UTF8_VALUE)
    @EnableApiLogger(name = "关闭申报",  type = "evaluation", buildDetail = EvaluationLogDetailBuilder.CloseBuilder.class)
    @ApiOperation("关闭申报")
    public ResultVo<Evaluation> closeDec(@PathVariable("id")String id){
        return ResultVo.success(service.closeDec(id));
    }

    @GetMapping(produces = APPLICATION_JSON_UTF8_VALUE)
    @ApiOperation("查询评测")
    public ResultPageVo<Evaluation> query(
            @ApiParam(value = "名称") @RequestParam(required = false) String name,
            @ApiParam(value = "状态") @RequestParam(required = false) String status,
            @RequestParam(defaultValue = "true") @ApiParam(value = "是否得到查询记录数") boolean isCount,
            @RequestParam(defaultValue = "0") @ApiParam(value = "查询页数") int page,
            @RequestParam(defaultValue = "15") @ApiParam(value = "查询每页记录数") int rows){

        Evaluation.Status s =   StringUtils.isBlank(status)? null: Evaluation.Status.valueOf(status);
        return new ResultPageVo.Builder<>(page, rows, service.query( name, s,page * rows, rows))
                .count(isCount, () -> service.count( name, s))
                .build();
    }

    @GetMapping(value = "active", produces = APPLICATION_JSON_UTF8_VALUE)
    @ApiOperation("查询激活评测列表")
    public ResultVo<List<Evaluation>> queryActive(){
        return ResultVo.success(service.queryActive());
    }

}
