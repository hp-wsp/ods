...
import com.ts.server.ods.taskcard.controller.manage.excel.TaskItemExcelReader;
import com.ts.server.ods.taskcard.controller.manage.form.TaskItemSaveForm;
import com.ts.server.ods.taskcard.controller.manage.form.TaskItemUpdateForm;
import com.ts.server.ods.taskcard.controller.manage.logger.TaskItemLogDetailBuilder;
import com.ts.server.ods.taskcard.controller.manage.vo.TaskItemVo;
...
public class TaskItemManController {
    private static final Logger LOGGER = LoggerFactory.getLogger(TaskItemManController.class);
    ...
    @Autowired
    public TaskItemManController(TaskItemService service, TaskCardService cardService, GradeRateService rateService) {
    ...
    public ResultVo<TaskItemVo> save(@Valid @RequestBody TaskItemSaveForm form){
        TaskCardItem t = service.save(form.toDomain());
    ...
    public ResultVo<TaskItemVo> update(@Valid @RequestBody TaskItemUpdateForm form){
        TaskCardItem t = service.update(form.toDomain());
        return ResultVo.success(new TaskItemVo(t));
    }
    ...
    public ResultVo<ImportVo> importItem(@RequestParam(value = "file") @ApiParam(value = "上传文件", required = true) MultipartFile file,
        ...
        try(InputStream inputStream= file.getInputStream()){
            ...
            cardService.updateItemStats(cardId);
