...
public class TaskItemService {
    ...
    private final TaskCardItemDao dao;
    ...
    @Autowired
    public TaskItemService(TaskCardItemDao dao, TaskCardService cardService,
    ...
    public TaskCardItem save(TaskCardItem t){
        ...
        cardService.updateItemStats(t.getCardId());

        return dao.findOne(t.getId());
    }

    @Transactional(propagation = Propagation.REQUIRED)
    public void importItem(TaskCard card, TaskCardItem t){
    ...
    private EvaItem importEvaItem(String evaId, TaskCardItem t){
        ...
        item.setResults(t.getResults().stream().map(TaskCardItem.TaskItemResult::getLevel).toArray(String[]::new));
    ...
    private void fillScore(TaskCardItem t, Map<String, Integer> rates){
        int score = t.getScore();

        for(TaskCardItem.TaskItemResult result: t.getResults()){
            int rate;
    ...
    @Transactional(propagation = Propagation.REQUIRED)
    public TaskCardItem update(TaskCardItem t){
        TaskCardItem o = get(t.getId());
        ...
        cardService.updateItemStats(o.getCardId());
        return dao.findOne(t.getId());
    }
    ...
    public TaskCardItem get(String id){
        try{
    ...
    public List<TaskCardItem> queryByCardId(String cardI){
    ...
    public List<TaskCardItem> query(String taskId, String num, String require, String grade, int offset, int limit){
