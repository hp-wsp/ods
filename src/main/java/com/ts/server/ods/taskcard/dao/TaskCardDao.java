...
public class TaskCardDao {
    ...
    public void updateEvaluation(String evaId, String evaName, boolean isDecOpen, boolean isGradeOpen){
        final String sql = "UPDATE t_card SET eva_name = ?, is_open = ?, is_open_grade = ? WHERE eva_id = ?";
        jdbcTemplate.update(sql, evaName, isDecOpen, isGradeOpen, evaId);
    }

    public void updateDecMember(String evaId, String userId, String username, String name){
        final String sql = "UPDATE t_card SET dec_username = ?, dec_name = ? WHERE eva_id = ? AND dec_id = ?";
        jdbcTemplate.update(sql, username, name, evaId, userId);
    }

    public void updateCompany(String evnId, String compId, String compName, String compGroup, int compGroupNum){
        final String sql = "UPDATE t_card SET company_name = ?, company_group = ?, company_group_num = ? " +
                "WHERE eva_id = ? AND company_id = ?";
        jdbcTemplate.update(sql, compName, compGroup, compGroupNum, evnId, compId);
    }

    public void updateAssManager(String evaId, String manId, String manUsername, String manName){
        final String sql = "UPDATE t_card SET ass_username = ?, ass_name = ? WHERE eva_id = ? AND ass_id = ?";
        jdbcTemplate.update(sql, manUsername, manName, evaId, manId);
    }
    ...
    public boolean hasAssManagerByEvaId(String evaId, String manId){
        final String sql = "SELECT COUNT(id) FROM t_card WHERE eva_id = ? AND ass_id = ?";
        Integer count = jdbcTemplate.queryForObject(sql, new Object[]{evaId, manId}, Integer.class);
        return count != null && count > 0;
    }

    public boolean hasDecMemberByEvaId(String evaId, String memId){
        final String sql = "SELECT COUNT(id) FROM t_card WHERE eva_id = ? AND dec_id = ?";
        Integer count = jdbcTemplate.queryForObject(sql, new Object[]{evaId, memId}, Integer.class);
        return count != null && count > 0;
    }
    ...
    public void updateItemStats(String id, int count, int score){
        final String sql = "UPDATE t_card SET item_count = ?, score = ? WHERE id = ?";
        jdbcTemplate.update(sql, count, score, id);
    }
