...
import com.ts.server.ods.taskcard.controller.manage.form.GradeForm;
import com.ts.server.ods.taskcard.controller.manage.logger.GradeLogDetailBuilder;
...
@ApiACL({"ROLE_GRADE"})
@Api(value = "/manage/grade", tags = "评分API接口")
public class GradeManController {
    private static final Logger LOGGER = LoggerFactory.getLogger(GradeManController.class);
    ...
    private final SmsSender taskCardSmsService;

    @Autowired
    public GradeManController(TaskCardService cardService, TaskItemService itemService,
                              DeclarationService decService, SmsSender taskCardSmsService) {
    ...
    @EnableApiLogger(name = "打分", buildDetail = GradeLogDetailBuilder.GradeBuilder.class)
    @ApiOperation("打分")
    public ResultVo<TaskCardItem> grade(@Valid @RequestBody GradeForm form){
        Credential credential = getCredential();
        TaskCardItem item = itemService.grade(form.getId(), form.getLevel(), form.getScore(), form.getRemark(), credential.getId());
    ...
    @EnableApiLogger(name = "退回", buildDetail = GradeLogDetailBuilder.BackBuilder.class)
    @ApiOperation("退回")
    ...
    @EnableApiLogger(name = "撤回退回", buildDetail = GradeLogDetailBuilder.CancelBackBuilder.class)
    @ApiOperation("撤回退回")
    ...
    @EnableApiLogger(name = "撤回退回", buildDetail = GradeLogDetailBuilder.FinishBuilder.class)
    @ApiOperation("完成")
    public ResultVo<TaskCard> finish(@PathVariable("id")String id){
        ...
        TaskCard newCard = cardService.finish(id);
        return ResultVo.success(newCard);
    }
    ...
    public void downland(@PathVariable("itemId") String itemId, HttpServletResponse response){
        TaskCardItem item = itemService.get(itemId);
