...
public class MainDecController {
    ...
    @Autowired
    public MainDecController(MemberService memberService, TokenService tokenService,
    ...
    public ResultVo<LoginVo<Member>> memberLogin(@Valid @RequestBody LoginForm form){
        ...
        Credential credential = new DecCredential(m.getId(), m.getUsername(), buildRoles(m), m.getCompanyId());
        String token = tokenService.generate(credential);

        return ResultVo.success(new LoginVo<>(token, m));
    }

    private List<String> buildRoles(Member m){
        return m.isManager()?
                Arrays.asList("ROLE_DEC_MANAGE", "ROLE_DECLARATION", GlobalRole.ROLE_AUTHENTICATION.name()):
